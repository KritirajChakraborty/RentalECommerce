...
export default function Navbar() {
  const { state, toggleBtn, close } = useContext(MobileMenuContext);
  ...
  return (
    <header className="max-w-screen-2xl mx-auto md:px-28 px-4 absolute top-0 left-0 right-0">
      <nav className="flex justify-between container sm:py-4 pt-6 pb-3">
        ...
        <a href="/">
          <h1 className="font-primary text-3xl font-bold sm:text-xl">
            RentExchange
          </h1>
        </a>
        ...
        <div className="sm:hidden">
          <button className="border-none" onClick={toggleBtn}>
            {state ? (
      ...
      <div className="pt-4">
        <ul className="lg:flex items-center justify-between gap-4 py-4 hidden">
          {navItems.map((item) => (
            <li
              key={item.id}
              className="text-lg text-Black font-primary hover:text-orange-500"
            >
      ...
      <div>
        <ul
          className={` bg-black text-white gap-3 rounded p-1 ${
            state ? "" : "hidden"
          }`}
        >
          {navItems.map((item) => (
            <li
              key={item.id}
              className="hover:text-orange-500 py-3 cursor-pointer pl-1"
            >
